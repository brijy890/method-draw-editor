...
// Function: svgedit.utilities.text2html
// Cross-browser compatible method of converting a string to an XML tree
// found this function here: http://groups.google.com/group/jquery-dev/browse_thread/thread/c6d11387c580a77f
svgedit.utilities.text2html = function(str) {
  var out;
  try{
    var dXML = (window.DOMParser)?new DOMParser():new ActiveXObject("Microsoft.XMLDOM");
    dXML.async = false;
  } catch(e){ 
    throw new Error("html Parser could not be instantiated"); 
  };
  try{
    out = dXML.parseFromString(str, "text/html");
  }
  catch(e){ throw new Error("Error parsing html string"); };
  return out;
};
