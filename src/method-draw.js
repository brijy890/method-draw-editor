...
(function() {
  ...
  if(!window.methodDraw) window.methodDraw = function($) {
    ...
    curConfig = {
      ...
      initStroke: {width: 2, color: '000', opacity: 1},
      initOpacity: 1,
      imgPath: 'images/',
      extPath: 'extensions/',
    ...
    Editor.addImage = function(url){
        var img = new Image();
        img.src = url;
        img.onload = function() {
          //alert(this.width + 'x' + this.height);
          imgWidth = curConfig.dimensions[0]/2;
          imgHeight = curConfig.dimensions[1]/2
          var id = svgCanvas.getNextId();
          cur_img = svgCanvas.addSvgElementFromJson({
            "element": "image",
            "attr": {
              "x": 50,
              "y": 50,
          ...
          svgCanvas.selectOnly([cur_img]);
        }      
    }

    Editor.loadTemplate = function(url){

      $.ajax({url: url, success: function(result){
        //console.log(String(result.documentElement));
        //loadSvgString(String(result));
        //svgCanvas.clear();
        //var temCont = document.getElementById('svgcontent');        
        //temCont.appendChild(result.documentElement);
        //svgCanvas.updateCanvas();
              ...
              loadSvgString(String(result));                
              updateCanvas();
              var temCont = $('#svgcontent');
              var htmlDom = svgedit.utilities.text2html(e.target.result);
              fObjColl = htmlDom.getElementsByTagName("foreignObject");
              temCont.find('foreignObject').each(function() {
                var obj = this;
                var child = obj.firstElementChild;
                obj.removeChild(child);
                //obj.appendChild(htmlNode.documentElement.childNodes[1].firstElementChild);
                for(i=0;i<fObjColl.length;i++){
                  if(fObjColl[i].id == this.id){
                    obj.appendChild(fObjColl[i].firstElementChild);
                  }
                }
              });        
      }});
    ...
    Editor.init = function() {
      ...
      $.svgIcons(curConfig.imgPath + 'svg_edit_icons.svg', {
        ...
        placement: {
          ...
          '#tool_path':'path',
          '#tool_text,#layer_rename,#tool_multitext':'text',
          '#tool_image':'image',
          '#tool_zoom':'zoom',
...
//            || elem && elem.tagName == "path" && !multiselected) { // This was added in r1430, but not sure why
  ...
  //              var ref_data = Actions.getButtonData(opts.button);
      ...
      var createBackground = function(fill) {
        ...
        var cutMargins = curConfig.cutMargins;
        if(cutMargins.length > 0){
            //console.log(cutMargins);
            for (var i = 0; i < cutMargins.length; i++) {
              // Do something with yourArray[i].
              svgCanvas.createLayer("margin_"+i);
              cur_shape = svgCanvas.addSvgElementFromJson({
                "element": "rect",
                "attr": {
                  "x": cutMargins[i]/2,
                  "y": cutMargins[i]/2,
                  "width": res.w-cutMargins[i],
                  "height": res.h-cutMargins[i],
                  "stroke": "#f00",
                  "stroke-width": "0.5",
                  "stroke-opacity": "0.5",
                  "id": "cut_margins_"+i,
                  "opacity": 1,
                  "fill": "none",
                  "style": "pointer-events:none; stroke-dasharray:3 3;"
                }
              });
              var crt_layer = svgCanvas.getCurrentDrawing().getCurrentLayer();
              crt_layer.setAttribute("data", "cut_margin");
              svgCanvas.setCurrentLayer("Layer 1");
              svgCanvas.setCurrentLayerPosition("1");
      ...
      //console.log(curConfig.cutMargins);
      //svgCanvas.setMarginBleed('#f00');
      ...
      */
      ...
      var clickMultiText = function(){
        if (toolButtonClick('#tool_multitext')) {
          svgCanvas.setMode('multiText');
        }
      };      
        ...
        window.svgAsDataUri(svg, {}, svgUri => {
          ...
          $image
              .on('load', () => {
                  ...
                  var dim = curConfig.dimensions;
                  var canvas = document.createElement('canvas'),
                      ctx = canvas.getContext('2d'),
                      doc = new jsPDF('l', 'pt',[dim[0],dim[1]]),
                      imgWidth = dim[0],
                      imgHeight = dim[1];
                  ...
                  ctx.drawImage(image, 0, 0, dim[0], dim[1]);

                  // Add the image to the pdf
                  var dataUrl = canvas.toDataURL('image/jpeg');
                  doc.addImage(dataUrl, 'JPEG', 0, 0, dim[0], dim[1]);
    ...
    //    setResolution(res.w * multiplier, res.h * multiplier, true);
        ...
        $('#zoom').text(Math.round(multiplier * 100)+"%");
...
//        var opacity = (picker == 'stroke' ? $('#stroke_opacity') : $('#fill_opacity'));
      ...
      var PaintBox = function(container, type) {
        ...
        if (type === 'canvas') {docElem.setAttribute('width',58);docElem.setAttribute('height',38);
        ...
        if(type === 'canvas') {this.rect.setAttribute('stroke', '#cfcfcf');this.rect.setAttribute('stroke-width', 1);}
      ...
      $('#tool_zoomin').mouseup( function(){
        if (updateCanvas) {
            zoomImage(1.2);
            //console.log(svgCanvas.getResolution());
        }
      });  

      $('#tool_zoomout').mouseup( function(){
        if (updateCanvas) {
            zoomImage(0.8);
            //console.log(svgCanvas.getResolution());
        }
      ...
      $('#tab_btn').mouseup(function(){
        $("#tab_cont").css("display", "block");
        $("#tab_cont").animate({left: "0px"});          
      });

      $('#tool_tab_close').mouseup(function(){
        //$("#tab_cont").css("display", "block");
        $("#tab_cont").animate({left: "-450px"});          
      });

      $('#menu_tabs > li').mouseup(function(){
          console.log($(this).attr('class'));
          $("#tab_contents > div").removeClass("on");
          switch ($(this).attr('class')) {
              case "shapes":
                    $("#selectedTab").css("transform", "translateY(0px)");
                    $("#tab_shapes").addClass("on");
                  break;
              case "images":
                    $("#selectedTab").css("transform", "translateY(80px)");
                    $("#tab_images").addClass("on");
                  break;
              case "text":
                    $("#selectedTab").css("transform", "translateY(160px)");
                    $("#tab_text").addClass("on");
                  break;
              case "patterns":
                    $("#selectedTab").css("transform", "translateY(240px)");
                    $("#tab_patterns").addClass("on");
                  break;
              case "uploads":
                    $("#selectedTab").css("transform", "translateY(320px)");
                    $("#tab_uploads").addClass("on");
                  break;
              default:
                  //c = "design_folder"
          }
    ...
    var changeCanvasSize = function(ctl){
      ...
      var Actions = function() {
        ...
        var tool_buttons = [
          ...
          {sel:'#tool_multitext', fn: clickMultiText, evt: 'click', key: ['M', true]},
          ...
          {sel:'#tool_pdf', fn: clickPdf, evt: 'mouseup'},
      ...
      if (window.FileReader) {
        ...
        //workarea[0].addEventListener('drop', load_image, false);
