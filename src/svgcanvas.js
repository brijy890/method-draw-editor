...
clearSvgContentElement();
  ...
  //var svgPrintMargins = svgdoc.createElementNS(svgns, "svg");
...
//          console.log('s',[start_x, start_y]);
      ...
      case "multiText":
        started = true;
        var fobjId = getNextId();
        /**/
        var newMultiText = addSvgElementFromJson({
        ...
        outerDiv.id = 'div_'+fobjId;
        var textDiv = document.createElement("div");
        textDiv.setAttribute( 'contenteditable', true );
        textDiv.textContent = 'Double click here to edit...';
        textDiv.id = 'div_div_'+fobjId;
        textDiv.setAttribute( 'align', 'left' );
        outerDiv.appendChild(textDiv);
        newMultiText.appendChild(outerDiv);

        //CKEDITOR.disableAutoInline = true;
        //CKEDITOR.inline(textDiv.id);
...
//              call("selected", [selected]);
      ...
      case "multiText":
        keep = true;
        //console.log("[element]ssss"+element);
...
/********************************* popup ckeditor init start ****************************/

    if(tagName  === 'foreignObject'){
      var ckElem = mouse_target.childNodes[0].id;
      $(document).on($.modal.BEFORE_CLOSE, function(event,modal){
            //console.log(ckElem);

            if(CKEDITOR.instances['div_'+ckElem]){CKEDITOR.instances['div_'+ckElem].destroy();};

            //console.log(modal.elm[0]);
            mouse_target.appendChild(modal.elm[0]);
            modal.elm[0].removeAttribute('class');
            modal.elm[0].removeAttribute('style');            
            $(document).unbind('$.modal.BEFORE_CLOSE');
            $(document).unbind('$.modal.OPEN');

      });
      $(document).on($.modal.OPEN, function(event,modal){
            var instanceExists = CKEDITOR.instances['div_'+ckElem];
            modal.elm[0].removeAttribute('style');
            if(!instanceExists){
                CKEDITOR.inline( 'div_'+ckElem, {
                  startupFocus: true
                } );
            }
          }
      ...
      $('#'+ckElem).modal({escapeClose: false,clickClose: false});

      $('.blocker').keydown(function(e) {
           e.stopPropagation();
      })
    } 
...
this.svgToString = function(elem, indent) {
  ...
  if (elem) {
    ...
    out.push("<"); out.push((elem.nodeName).toLowerCase());
...
//      }
      ...
      if(elem.getAttribute('data') === 'cut_margin') return;
    ...
    if (elem.hasChildNodes()) {
      ...
      out.push("</"); out.push((elem.nodeName).toLowerCase()); out.push(">");
    } else {
      out.push("/>");
    }
  }
...
this.save = function(opts) {
  ...
  //console.log(str);
  str = str.replace(/foreignobject/g, "foreignObject");
  //console.log(str);
...
this.setMode = function(name) {
  ...
  cur_properties = (selectedElements[0] && (selectedElements[0].nodeName == 'text' || selectedElements[0].nodeName == 'foreignObject')) ? cur_text : cur_shape;
  current_mode = name;
};
...
/***************************** Print agile *****************************/
// Function: setMarginBleed
// Set the margin and bleed of the editor (NOT the actual document)
//
// Parameters:
// color - String with fill color to apply
// url - URL or path to image to use
/*
this.setMarginBleed = function(color) {
  var svgPrintMarginCont =  getElem('svgprintmargins');

  var margins = svgdoc.createElementNS(svgns, 'rect');
  margins.setAttributeNS(null, 'x', 0);
  margins.setAttributeNS(null, 'y', 0);
  margins.setAttributeNS(null, 'height', '50');
  margins.setAttributeNS(null, 'width', '50');
  margins.setAttributeNS(null, 'stroke', '#f00');
  margins.setAttributeNS(null, 'stroke-width', '2');
  margins.setAttributeNS(null, 'stroke-opacity', '2');
  margins.setAttributeNS(null, 'fill', 'none');
  svgPrintMarginCont.appendChild(margins);
}
*/
/***************************** Print agile *****************************/
